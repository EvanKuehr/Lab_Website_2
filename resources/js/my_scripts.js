...
function loadStatsPage()
{
		...
		var winCounter = 0;
		var lossCounter = 0;
		for(row = 2; row < table1.rows.length; row++)
		{
			homeScore = table1.rows[row].cells[2].innerHTML;
			oppScore = table1.rows[row].cells[3].innerHTML;
			if(parseInt(homeScore) > parseInt(oppScore) )  //Buffs won
			{
				table1.rows[row].cells[4].innerHTML = "CU Boulder";
				winCounter++;
			}
			else //Opposing team won
			{
...
function loadPlayersPage()
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		anchorTag.innerText = players[i].name;
		funcCallString = "switchPlayers(" + i + ")";
		anchorTag.setAttribute('onClick',funcCallString);
